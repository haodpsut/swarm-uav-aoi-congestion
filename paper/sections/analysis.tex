...
\;+\; \underbrace{\tfrac{2\,\mathrm{dist}}{V} + \tau_{\mathrm{ch}}}_{c_0},
\end{equation}
...
per-source operating rate $\lambda=1/\mathrm{up\_time}$, port rate
$\mu=1/\tau_{\mathrm{ch}}$), and $c_0$ collects the recharge round trip and the
...
$\Aoi=c_1(1)+2\,\mathrm{dist}/V+\tau_{\mathrm{ch}}$ depends on placement only
...
\text{(B) split:}\quad
&\Aoi_{\mathrm B}(M) = t_{\mathrm{loop}} + \delta + \Wq\!\left(\tfrac{M}{2},c\right) + c_0 ,
\end{align}
