...
its hover points, initialized at the $k$-means centroids of the sub-field. Within
a cyclic order the closed-tour length is differentiable in the coordinates, so we
minimize it by gradient descent. To also optimize the visiting order, and not
freeze it at initialization, we alternate the continuous and combinatorial parts:
we run a block of gradient steps on the coordinates, then reorder the current
hover points by a $2$-opt improvement of the tour, then repeat. This is a
coordinate descent between the continuous positions and the discrete sequence,
so the order adjusts as the positions move. The objective is the total
closed-tour length plus a coverage penalty,
\begin{equation}
\begin{split}
  \mathcal{L}
  = \sum_{m\in\mathcal{M}}
...
\eqref{eq:cetsp_loss} by Adam over the hover-point coordinates, interleaved with
the $2$-opt reordering above. All UAVs of a scenario are stacked into one padded,
masked batch and the gradient steps run in parallel on a GPU, so the whole swarm
(and, by stacking seeds, whole sweeps) is optimized together. The block returns each UAV's optimized tour length, hence its
