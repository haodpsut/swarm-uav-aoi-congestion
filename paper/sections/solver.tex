...
use, and the loop drives the peak AoI down monotonically until no block can
improve it. We describe each block, then the overall algorithm.
...
\begin{equation}
  \mathcal{L}
  = \sum_{m\in\mathcal{M}}
    \Bigg[\underbrace{\sum_{j=1}^{n_m}\big\|\mathbf{p}_{m,j{+}1}-\mathbf{p}_{m,j}\big\|}_{\text{tour length}}
    + \lambda_{\mathrm{cov}}
...
fixed point \eqref{eq:pn}--\eqref{eq:wq} from the number of assigned UAVs $N_s$,
...
load-balancing local search: we repeatedly reassign the single UAV whose move
gives the largest reduction in peak AoI \eqref{eq:peak_m}, and stop when no
single reassignment improves it. Because the peak AoI is set by the worst
...
\FOR{each site subset $\mathbf{x}$ of size $S_{\max}$ in $\mathcal{S}$}
  \FOR{each port split $\mathbf{c}$ with $\sum_s c_s = C_{\mathrm{tot}}$}
    ...
    \REPEAT
      \STATE among all single-UAV reassignments, pick the one that most reduces
        the peak AoI \eqref{eq:peak_m}, evaluating $\Wq$ by \eqref{eq:wq}
      \STATE apply it if it improves the peak AoI
    \UNTIL{no single reassignment improves the peak AoI}
...
zero) and there are finitely many assignments. Each block leaves the peak AoI no
higher than it found it: the port and assignment blocks accept a change only when
it lowers \eqref{eq:peak_m}, and the placement search keeps the best subset seen.
The peak AoI is therefore non-increasing across the alternation and the procedure
converges to a coordinate-wise local optimum. We do not claim global optimality:
the trajectory order is fixed and the assignment search is greedy, so the result
is the best configuration reachable by improving one block at a time from the
initialization.
