...
deviation shown where relevant. All trajectory optimisation runs on an
NVIDIA RTX~4090.
...
patrolling for a randomised operating time and travelling to the station to
...
by about $5.6\times$, because it ignores the closed population: only $M$ UAVs
...
grows with the charging capacity, as summarised in Table~\ref{tab:mstar}, which
...
$20/20$ instances, by $14.8\%$ on average, and it selects \emph{different}
station sites in $18/20$ instances (Fig.~\ref{fig:prop2}). This confirms
...
a nearest-neighbour patrol. Table~\ref{tab:ablation} reports the ablation at a
...
$p{=}8.2\times10^{-5}$ against the strongest baseline, so the improvement is
...
\begin{table}[t]
  ...
  \begin{tabular}{lc}
    ...
    No CETSP (nearest-neighbour patrol)       & 42.8 \\
...
That artifact came from the open queue overpenalising split stations. With the
...
only to minimise flight detour. The coverage-optimal placement concentrates the
...
Third, the queueing model matters as much as the optimiser. Treating the
