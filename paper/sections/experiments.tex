...
open stations and $C_{\mathrm{tot}}{=}4$ ports. In the joint comparison we use a
swarm of $M{=}12$ UAVs, which is not an arbitrary cap but the AoI-optimal swarm
size $M^\star$ for this charging capacity, as the provisioning law establishes
(Table~\ref{tab:mstar}); running the comparison at $M^\star$ is the fair
operating point, since a larger swarm at fixed capacity is past the congestion
threshold of Proposition~\ref{prop:ushape}. The scaling study in
Section~\ref{subsec:scale} covers genuinely large swarms, up to $M{=}60$, by
growing the charging capacity with the swarm. Every reported value is a mean over
$20$ to $30$ random sensor fields (fixed seeds), with the standard deviation
shown where relevant. All trajectory optimization runs on an NVIDIA RTX~4090.
...
finite-source model throughout. The finite-source $M/M/c//N$ model assumes
exponential service and is thus a conservative bound: it exceeds the real wait by
about $4.5\times$. Replacing the exponential service by a deterministic-service
$M/D/c//N$ approximation, obtained with the standard service-variability
correction (the wait scales with $(1+c_s^2)/2$, where $c_s$ is the squared
coefficient of variation of the service time, so $c_s^2{=}0$ halves the wait),
tightens this to about $2.3\times$ (Fig.~\ref{fig:des}). The residual gap is due
to the near-deterministic \emph{operating} process, which a service-only
correction does not capture; since both models are upper bounds, the reported
AoI values remain safe rather than optimistic.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{../results/fig_des_validation.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{../results/fig_prop1_phys.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.86\columnwidth]{../results/fig_prop1_mstar.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{../results/fig_prop2.pdf}
...
trajectory contributes about $3.9\%$, and the contention-aware placement
contributes substantially more (about $18\%$ over the coverage-optimal
placement). A Wilcoxon signed-rank test gives $p{=}1.9\times10^{-6}$ against the
closest competitor, the no-CETSP ablation, so the improvement is statistically
significant.
...
\begin{table}[t]
  ...
  \begin{tabular}{lc}
    ...
    Method & Peak AoI (min) \\
    \midrule
    Proposed (CETSP + traffic-optimal)       & \textbf{41.1} \\
    No CETSP (nearest-neighbor patrol)       & 42.8 \\
    Single pooled station \cite{wei2024}      & 46.2 \\
    Coverage-optimal placement                & 50.4 \\
    Round-robin assignment                    & 56.7 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\columnwidth]{../results/fig_ablation.pdf}
...
and the gain grows with the field (from $4.1\%$ at $5$~km to $12.2\%$ at
$20$~km) because travel cost dominates as the field grows. The choice of queue
model is decisive here. Had the charging system been modeled as an open $M/M/c$
queue, a spurious crossover would appear in which the single pooled station wins
in small fields, because the open queue overpenalizes split stations. The
DES-validated finite-source model, whose wait saturates rather than diverging,
removes this artifact, and the distributed placement is preferred throughout.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{../results/fig_crossover.pdf}
...
\subsection{Scaling to a large swarm}
\label{subsec:scale}
A swarm is only meaningful at larger scale, so we scale the fleet to $M=60$ UAVs
over a $20$~km field with $K=8M$ sensors, and we grow the charging capacity with
the swarm ($C_{\mathrm{tot}}=\lceil M/3\rceil$ ports) as the provisioning law of
Proposition~\ref{prop:ushape} requires. Fig.~\ref{fig:scale} shows that the peak
AoI stays controlled as the swarm grows (from $50.6$~min at $M=20$ to $47.6$~min
at $M=60$), because the sensors and the charging capacity scale together, and
that the proposed contention-aware placement keeps its advantage over the single
pooled station and the coverage-optimal placement across the whole range, by
$10.2$ to $12.7\%$. The design therefore applies to genuine swarms, not only to
small fleets.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\columnwidth]{../results/fig_scale.pdf}
  \caption{Scaling to a large swarm ($M$ up to $60$, $K=8M$ sensors, capacity
  $C_{\mathrm{tot}}=\lceil M/3\rceil$ growing with the swarm). The proposed
  placement keeps its advantage over the baselines at scale.}
  \label{fig:scale}
\end{figure}

\subsection{Solver optimality}
The placement and assignment blocks are greedy, so we ask how far they are from
the true optimum. On small instances the exact optimum is computable by
enumerating every open-site subset, every port split, and every
UAV-to-station assignment; we run the greedy solver and the enumeration on
identical per-UAV states, so the gap measures only the combinatorial heuristic.
Table~\ref{tab:optgap} reports the result over $15$ seeds. The greedy placement
and assignment are \emph{exactly} optimal for $M\le 6$ (they match the enumerated
optimum on every seed), and remain within $0.11\%$ on average and below $1\%$ in
the worst case at $M=8$. The heuristic therefore matches the optimum wherever an
exact comparison is tractable, which supports its use at the
larger swarm sizes where enumeration is infeasible.

\begin{table}[t]
  \centering
  \caption{Optimality gap of the greedy solver versus the exact optimum.}
  \label{tab:optgap}
  \begin{tabular}{lcccccc}
    \toprule
    Swarm size $M$        & 3 & 4 & 5 & 6 & 7 & 8 \\
    \midrule
    Mean gap (\%)         & 0 & 0 & 0 & 0 & 0.03 & 0.11 \\
    Optimal seeds         & 15/15 & 15/15 & 15/15 & 15/15 & 13/15 & 10/15 \\
    \bottomrule
  \end{tabular}
\end{table}

\subsection{Robustness}
Fig.~\ref{fig:sensitivity} sweeps the main scenario parameters at $L=14$~km. The
gain of the proposed placement over the single pooled station is stable across
the collection radius ($11.2$ to $11.4\%$ for $r_c$ from $100$ to $400$~m) and
the charge time ($8.5$ to $13.5\%$). It is also larger, not smaller, on a
non-uniform field: when the sensors form clusters the gain rises to $19.0\%$,
because concentrated recharge traffic makes load balancing more valuable. The
advantage is therefore robust and, if anything, strengthens in realistic
non-uniform deployments.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{../results/fig_sensitivity.pdf}
  \caption{Robustness of the proposed method's gain over the single pooled
  station to the collection radius $r_c$, the charge time $\tau_{\mathrm{charge}}$,
  and a uniform versus clustered sensor field ($L=14$~km, $M=12$).}
...
the peak AoI is non-monotone in the swarm size, there is an optimal swarm size
